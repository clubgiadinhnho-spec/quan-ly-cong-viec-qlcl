    ...
    <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6">
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-4">
          <h2 className="text-[20px] font-black text-blue-900 tracking-tight uppercase leading-none whitespace-nowrap notranslate" translate="no">
            {title}
          </h2>
          {effectiveUser ? (
            ...
            <div className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[10px] font-bold uppercase tracking-wider whitespace-nowrap">
              {badge}
            </div>
          ) : null}
        </div>

        {/* Online Users Display - Desktop ONLY */}
        {sortedDisplay.length > 0 && (
          <div className="hidden md:flex items-center gap-3 border-l border-gray-100 pl-6 ml-2 print:hidden no-print">
      ...
      <div className="flex gap-3 items-center">
        ...
        {onAction && actionLabel && (
          <button 
            onClick={onAction}
            className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-lg shadow-sm hover:bg-blue-700 flex items-center gap-2 transition-all whitespace-nowrap flex-shrink-0 min-w-fit cursor-pointer"
          >
            {Icon && <Icon size={14} />}
            {actionLabel}
          </button>
