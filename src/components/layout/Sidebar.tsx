...
export const Sidebar = ({ 
  ...
  return (
    <aside className={`w-72 ${currentColor.bg} border-r border-gray-200 flex flex-col h-screen sticky top-0 transition-all duration-500`}>
      <div className="p-6 flex flex-col h-full">
        <div className="flex flex-col gap-1 mb-6 px-0.5">
          <div className={`flex items-center gap-3 p-3 ${isDark ? 'bg-white/10' : 'bg-gradient-to-br from-blue-50 to-indigo-50/50'} rounded-2xl border ${isDark ? 'border-white/10' : 'border-blue-100/50'} shadow-sm relative overflow-hidden group`}>
          ...
          {[
            ...
            >
              ...
              {item.count !== undefined && item.count > 0 && (
                <span className={`text-[11px] font-black min-w-[22px] h-5.5 px-1.5 rounded-full flex items-center justify-center border-2 ${isDark ? 'border-transparent' : 'border-white'} shadow-md ${
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar pb-6 pt-4">
          <button 
            ...
            className={`w-full flex items-center gap-2.5 px-3.5 group hover:bg-rose-50/10 py-3 rounded-xl transition-all ${activeTab === 'group_chat' ? (isDark ? 'bg-white/20 ring-1 ring-white/20' : 'bg-rose-50/50 ring-1 ring-rose-100') : ''}`}
          >
            ...
            <div className="flex-1 min-w-0 text-left flex items-center justify-between gap-1">
              <div className="min-w-0">
                <p className={`text-sm font-black uppercase truncate transition-colors ${activeTab === 'group_chat' ? (isDark ? 'text-white' : 'text-rose-600') : (isDark ? 'text-white/80 group-hover:text-white' : 'text-gray-700 group-hover:text-rose-600')}`}>Room Thảo Luận</p>
                <p className={`text-[10px] font-bold uppercase tracking-widest truncate ${isDark ? 'text-white/40' : 'text-gray-400'}`}>Cộng đồng QLCL</p>
              </div>
      ...
      <div className={`px-4 py-3 border-t ${isDark ? 'border-white/10' : 'border-gray-100/50'} flex justify-center`}>
        <div className={`flex items-center gap-2.5 ${isDark ? 'bg-white/10' : 'bg-gray-100/50'} p-1.5 rounded-full border ${isDark ? 'border-white/10' : 'border-gray-200/50'}`}>
          {COLOR_OPTIONS.map((option) => (
            <button
              key={option.id}
              onClick={() => setSidebarColor(option.id)}
              className={`w-4 h-4 rounded-full border shadow-sm transition-all duration-300 ${option.dot} ${
      ...
      <div className={`mt-0 p-4 border-t ${isDark ? 'border-white/10' : 'border-gray-100'} ${isDark ? 'bg-white/5' : 'bg-gray-50/50'}`}>
