    ...
    <AnimatePresence>
      ...
      {isMinimized && variant === 'bubble' ? (
        ...
        >
          ...
          <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[8px] py-0.5 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap font-bold pointer-events-none z-10 shadow-lg">
            {otherUser.name}
          </div>
        </button>
      ) : (
          ...
          <div className={`p-4 ${isMinimized ? 'h-full' : 'border-b border-gray-100 bg-gray-50/50 backdrop-blur-sm'} flex items-center justify-between cursor-pointer`} onClick={() => isMinimized && onMinimizeChange?.(false)}>
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h3 className={`${isMinimized || variant === 'bubble' ? 'text-[11px]' : 'text-sm'} font-black text-gray-800 uppercase tracking-tighter truncate w-32`}>{otherUser.name}</h3>
                {!isMinimized && variant !== 'bubble' && (
